export function Parallax(clip) {
  var parallax = clip.querySelectorAll('*[data-parallax="true"]')
  var parallaxDetails = []
  var sticky = false
  ...
  clip.addEventListener('scroll', function () {
    for (var i = 0; i < parallaxDetails.length; i++) {
      ...
      var threshold = 200
      var visible =
        parallaxStart - threshold - clip.clientHeight < clip.scrollTop && parallaxEnd + threshold > clip.scrollTop
...
function onResize(details) {
  for (var i = 0; i < details.length; i++) {
    ...
    details[i].node.style.transform =
      'scale(' + (1 - depth) + ') translate3d(' + dx + 'px, ' + dy + 'px, ' + depth + 'px)'
  }
}
